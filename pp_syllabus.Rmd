...
insight_readings <- tribble(
  ~reading_text, ~reading_link, ~week_assigned,
  #--------------------------------
...
group_counts <- dated_readings %>% 
  arrange(class_day) %>% 
  group_by(week_assigned) %>% 
  summarize(group_size = n())

group_indexes <- setNames(group_counts$group_size, group_counts$week_assigned)
...
dated_readings %>% 
  ...
  kable("html", escape = FALSE) %>% 
  kable_styling(bootstrap_options = c("responsive", "striped")) %>% 
  group_rows(index = group_indexes)
