...
Students will identify and participate in an active peer production community of their choosing. The activity does not have to be software development and does not have to use `git` or `github`. For example, it could be reading and contributing a chapter of a book to [LibriVox](https://librivox.org/). If you are already involved in a peer production project you may use that, but most will be entering afresh. There are three graded components, each to be submitted via a pull request.
